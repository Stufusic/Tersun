...
enum class DispatchMode {
    FUNCTION_POINTER = 0, // Variant 0: Baseline 1.0.3 (Function Pointer array + per-opcode try/catch)
    ...
    REGISTER_CACHED  = 3  // Variant 3: Direct threaded + Register-resident hot IP/SP + chunk-level try/catch
};
...
public:
    ...
    void run_function_pointer(const Chunk& chunk);
    void run_switch(const Chunk& chunk);
    void run_threaded(const Chunk& chunk);
    void run_cached(const Chunk& chunk);
    void run_optimized(OptimizedChunk& opt);

    void set_dispatch_mode(DispatchMode mode) { dispatch_mode_ = mode; }
    DispatchMode dispatch_mode() const { return dispatch_mode_; }

    // Gate 4 Ablation and Telemetry Controls
...
private:
    ...
    void handle_mod(const Chunk& chunk);
    void handle_bit_and(const Chunk& chunk);
    void handle_bit_or(const Chunk& chunk);
    void handle_bit_xor(const Chunk& chunk);
    void handle_shl(const Chunk& chunk);
    void handle_shr(const Chunk& chunk);
    ...
    DispatchMode dispatch_mode_{DispatchMode::REGISTER_CACHED};
