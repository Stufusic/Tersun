...
enum class OpCode : uint8_t {
    ...
    OP_MOD             = 0x29, // Integer & Tryte Modulo with zero-guard
    OP_BIT_AND         = 0x2A, // Bitwise AND / Tritwise Kleene Min
    OP_BIT_OR          = 0x2B, // Bitwise OR / Tritwise Kleene Max
    OP_BIT_XOR         = 0x2C, // Bitwise XOR / Tritwise GF(3) Addition
    OP_SHL             = 0x2D, // Shift Left (<<)
    OP_SHR             = 0x2E, // Shift Right (>>)
    ...
    // ========================================================================
    // Gate 4: Tier-1 Adaptive VM Execution & Specialized Opcodes (0xB0 - 0xDF)
    // ========================================================================
    // Tier A: Superinstructions (Direct Local Slot Access & Fast Increment)
    OP_LOAD_LOCAL_0    = 0xB0,
    OP_LOAD_LOCAL_1    = 0xB1,
    OP_LOAD_LOCAL_2    = 0xB2,
    OP_LOAD_LOCAL_3    = 0xB3,
    OP_STORE_LOCAL_0   = 0xB4,
    OP_STORE_LOCAL_1   = 0xB5,
    OP_STORE_LOCAL_2   = 0xB6,
    OP_STORE_LOCAL_3   = 0xB7,
    OP_INCR_LOCAL_IMM  = 0xB8, // uint16_t slot, int16_t imm

    // Tier B: Fast Array Indexing
    OP_GET_INDEX_ARRAY = 0xB9,
    OP_SET_INDEX_ARRAY = 0xBA,

    // Tier C: Fused Loop Iteration
    OP_LOOP_RANGE_FAST = 0xBB, // uint16_t var_slot, uint16_t stop_slot, uint16_t step_slot, int16_t rel_jump

    // Adaptive Quickening Sites
    OP_QUICK_ADD_INT   = 0xBC,
    OP_QUICK_SUB_INT   = 0xBD,
    OP_QUICK_MUL_INT   = 0xBE,
    OP_QUICK_LT_INT    = 0xBF,
    OP_QUICK_LE_INT    = 0xC0,
    OP_QUICK_EQ_INT    = 0xC1,

    // Shape-based Inline Cache
