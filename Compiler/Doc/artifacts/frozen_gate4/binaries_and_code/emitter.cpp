...
std::string_view opcode_name(OpCode op) {
    switch (op) {
        ...
        case OpCode::OP_CALL_INDIRECT: return "OP_CALL_INDIRECT";
        case OpCode::OP_LOAD_LOCAL_0: return "OP_LOAD_LOCAL_0";
        case OpCode::OP_LOAD_LOCAL_1: return "OP_LOAD_LOCAL_1";
        case OpCode::OP_LOAD_LOCAL_2: return "OP_LOAD_LOCAL_2";
        case OpCode::OP_LOAD_LOCAL_3: return "OP_LOAD_LOCAL_3";
        case OpCode::OP_STORE_LOCAL_0: return "OP_STORE_LOCAL_0";
        case OpCode::OP_STORE_LOCAL_1: return "OP_STORE_LOCAL_1";
        case OpCode::OP_STORE_LOCAL_2: return "OP_STORE_LOCAL_2";
        case OpCode::OP_STORE_LOCAL_3: return "OP_STORE_LOCAL_3";
        case OpCode::OP_INCR_LOCAL_IMM: return "OP_INCR_LOCAL_IMM";
        case OpCode::OP_GET_INDEX_ARRAY: return "OP_GET_INDEX_ARRAY";
        case OpCode::OP_SET_INDEX_ARRAY: return "OP_SET_INDEX_ARRAY";
        case OpCode::OP_LOOP_RANGE_FAST: return "OP_LOOP_RANGE_FAST";
        case OpCode::OP_QUICK_ADD_INT: return "OP_QUICK_ADD_INT";
        case OpCode::OP_QUICK_SUB_INT: return "OP_QUICK_SUB_INT";
        case OpCode::OP_QUICK_MUL_INT: return "OP_QUICK_MUL_INT";
        case OpCode::OP_QUICK_LT_INT: return "OP_QUICK_LT_INT";
        case OpCode::OP_QUICK_LE_INT: return "OP_QUICK_LE_INT";
...
Chunk BytecodeEmitter::compile(const Program& program) {
    ...
    chunk_.function_frame_sizes.clear();
    next_fn_index_ = 0;
    class_fields_.clear();
    ...
    chunk_.toplevel_frame_size = static_cast<size_t>(next_local_slot_ + 16);
...
void BytecodeEmitter::emit_fn_decl(const FnDeclStmt& stmt) {
    ...
    symbol_table_.exit_scope();
    if (func_entry < chunk_.function_frame_sizes.size()) {
        chunk_.function_frame_sizes[func_entry] = next_local_slot_ + 8;
    }
...
void BytecodeEmitter::emit_binary(const BinaryExpr& expr) {
    ...
    switch (expr.op) {
        ...
        case BinaryOp::MOD: chunk_.write_opcode(OpCode::OP_MOD, expr.loc.line); break;
        case BinaryOp::BIT_AND: chunk_.write_opcode(OpCode::OP_BIT_AND, expr.loc.line); break;
        case BinaryOp::BIT_OR: chunk_.write_opcode(OpCode::OP_BIT_OR, expr.loc.line); break;
        case BinaryOp::BIT_XOR: chunk_.write_opcode(OpCode::OP_BIT_XOR, expr.loc.line); break;
        case BinaryOp::SHL: chunk_.write_opcode(OpCode::OP_SHL, expr.loc.line); break;
        case BinaryOp::SHR: chunk_.write_opcode(OpCode::OP_SHR, expr.loc.line); break;
...
std::vector<std::string> BytecodeEmitter::collect_lambda_captures(
    ...
    std::function<void(Stmt*)> walk_stmt;
    ...
    walk_expr = [&](Expr* e) {
        ...
        std::visit([&](auto& x) {
            ...
            if constexpr (std::is_same_v<T, IdentifierExpr>) {
                ...
                for (const auto& p : x.params) declared.insert(p.name);
                walk_stmt(x.body);
            }
        }, e->data);
    };
...
void BytecodeEmitter::emit_lambda(const LambdaExpr& expr) {
    ...
    symbol_table_.exit_scope();
    if (fn_idx < chunk_.function_frame_sizes.size()) {
        chunk_.function_frame_sizes[fn_idx] = next_local_slot_ + 8;
    }
    next_local_slot_ = saved_local_slot;
    chunk_.patch_jump(jump_over);
...
void BytecodeEmitter::emit_struct_decl(const StructDeclStmt& stmt) {
    ...
    for (const auto& m : stmt.methods) {
        ...
        chunk_.write_opcode(OpCode::OP_RET, stmt.loc.line);
        symbol_table_.exit_scope();
        if (fn_entry < chunk_.function_frame_sizes.size()) {
            chunk_.function_frame_sizes[fn_entry] = next_local_slot_ + 8;
        }
